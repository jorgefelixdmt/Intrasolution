...
strSQL = "pr_Curso_En_Programado_List " &  Id_Unidad
wRsCurso.Open strSQL, oConn



'Elimina el archivo excel 
NameFile1 ="PARTICIPANTE-" & Id_Unidad & ".xls"
...
strConnQuery = Application(wEmpresa)
oConn.Open(strConnQuery)
strSQL = "pr_Borra_Tmp_Carga_Asistencia_Participante " & Id_Unidad
    ...
    Select Case Acc
        ...
        Case "Load"
	        ...
	        if wErrorFile = "" then
                ...
                strSQL = "pr_valida_carga_participante " & Id_Unidad & "," & wId_Carga
                ...
                if cint(w_NumErrores) > 0 then
                    ...
                    strSQL = "pr_inserta_carga_participante " & Id_Unidad & "," & wId_Carga
...
<title>eco2biz : Carga Masiva Participante</title>
...
<tr>
	<td width="85%" class="TxtCab"><b>&nbsp;Carga Masiva :	Participante
    ...
    <form Name="frmParticipante" Action="asistencia_Participante.asp" Method="POST"  ENCTYPE="multipart/form-data">
    ...
    <td>
          <table border="0" cellspacing="0" cellpadding="0" width="600" bgcolor="#ffffff" class=dos>
                <tr>
				      <td  colspan=2 height=30 align="Left" valign=middle class=Header>&nbsp;<b>Carga de Participante</b></td>
                </tr>
			    <tr>
                ...
                <tr>
				      ...
				      (*) Especificaciones del Archivo (<a href="ayuda/Manual_Participante_Carga.pdf"  target="_blank" ><b>Descargar</b></a>)<br />
    				   Plantilla de Ejemplo (<a href="ayuda/Modelo_Participante_Carga.xls" target="_blank" ><b>Descargar</b></a>)
				      </td>
			    </tr>
...
<script LANGUAGE="JavaScript">
	...
	function EjecutaAccion(Acc)
	{
        ...
        {
		    frmParticipante.Acc.value=Acc; 
		    frmParticipante.submit();
		 }
		 else{
...
Sub CargaData()
        ...
        strSQL = strSQL + "'PARTICIPANTE - Unidad:" & Id_Unidad & "' "
        ...
        Do While Not objRS.Eof
            ...
            '**** carga a tabla temporal
                strSQL = "set dateformat ymd insert into cap_asistencia_temporal (fb_uea_pe_id,cap_curso_id,dni_empleado,nombre_completo,num_row,id_carga,fb_usuario_id,tipo_carga)"
...
{
    ret = window.open("Asistencia_Participante_Errores.asp?EMPRESA=<%=wEmpresa%>&Id_Unidad=<%=Id_Unidad%>&Id_Carga=<%=wId_Carga%>", "Nombre", "width=750,height=420,top=100,left=180,scrollbars=1");
