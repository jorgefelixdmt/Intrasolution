...
<%
	...
	wSQL = "pr_inc_graf_pendientes_responsable " & wId_Usuario & "," & wTipoInc & ",'" & wCodAmb & "'"
	...
	For Each fld In wRs.Fields 
		If contador > 0 Then
			wResp = wResp & """" & fld.name & ""","
		End If
		contador = contador + 1
	Next
	
	If contador > 0 Then
		wResp = Left(wResp,len(wResp)-1)
	End If
...
var config_<%=wCodigo%> = {
    ...
    title: {
        text: 'Incidentes Pendientes por Responsable',
		style: {
		  fontSize: '16px'
		}
    },
