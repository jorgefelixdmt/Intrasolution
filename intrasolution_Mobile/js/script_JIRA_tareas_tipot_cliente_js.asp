...
%>
		...
		function load_<%=wCodigo%>(){
			var cliente = document.getElementById("Cliente_<%=wCodigo%>").value;
			var estado = document.getElementById("Estado_<%=wCodigo%>").value;

			$.ajax({
				url: "js/script_JIRA_tareas_tipot_cliente_Load_js.asp?Empresa=<%=wEmpresa%>&Id_Unidad=<%=wId_Unidad%>&Id_Usuario=<%=wId_Usuario%>&inicio=<%=inicio%>&fin=<%=fin%>" + "&Cliente=" + cliente + "&Estado=" + estado,
				type: "POST",
				dataType: "json",
