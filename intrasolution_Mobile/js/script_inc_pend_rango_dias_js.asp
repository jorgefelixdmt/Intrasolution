...
<%
	...
	wSQL = "pr_inc_graf_pendientes_dias " & wId_Usuario & "," & wTipoInc & ",'" & wCodAmb & "'"
	...
	For Each fld In wRs.Fields 
		If contador > 0 Then
			wRango = wRango & """" & fld.name & ""","
		End If
		contador = contador + 1
	Next
	
	If contador > 0 Then
		wRango = Left(wRango,len(wRango)-1)
	End If
...
var config_<%=wCodigo%> = {
    ...
    title: {
        text: 'Incidentes Pendientes por Rango de Días',
		style: {
		  fontSize: '16px'
		}
    },
